...
export default function ExerciseDashboard() {
  ...
  const handleToggleAsset = useCallback(async (taskId: string, assetId: string, currentAssetIds: string[] = []) => {
    try {
      ...
      const isLinked = currentAssetIds.includes(assetId);

      await updateDoc(taskRef, {
        assetIds: isLinked ? arrayRemove(assetId) : arrayUnion(assetId),
  ...
  return (
    <div className="min-h-screen bg-[#f4f7f6] p-4 md:p-8">
          ...
          <div className="grid md:grid-cols-2 gap-4">
            {sortedTasks.map((task: any) => (
              <TaskCard
                ...
                onOpenMapper={(id) => setActiveMapperTaskId(id)}
              />
            ))}
          </div>
        </div>
      ...
      {activeMapperTaskId && (
        <AssetMapper
          selectedAssetIds={tasks.find((t: any) => t.taskId === activeMapperTaskId)?.assetIds || []}
          onToggleAsset={(assetId) => handleToggleAsset(
            activeMapperTaskId,
            assetId,
            tasks.find((t: any) => t.taskId === activeMapperTaskId)?.assetIds || []
          )}
          onClose={() => setActiveMapperTaskId(null)}
        />
      )}
    </div>
  );

...
export interface ExerciseTask {
  ...
  notes?: string;
  metadata?: Record<string, any>; // Generic metadata for extensions
}
...
/**
 * Phase 3: Webhook Engine - Webhook Configuration Schema
 * Collection: /webhook_configs/{configId}
 */
export interface WebhookConfig {
  configId: string;
  tenantId: string; // Critical for isolation
  type: 'inbound' | 'outbound';
  url?: string; // Only for outbound
  secret: string; // HMAC or shared secret for validation
  active: boolean;
  events?: string[]; // e.g., ['task.failed', 'exercise.completed']
  metadata?: Record<string, any>;
  createdAt: FieldValue | Date;
  updatedAt: FieldValue | Date;
}
